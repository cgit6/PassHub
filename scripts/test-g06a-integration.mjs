...
const projectName = 'passhub-g06a';
...
const databasePrefix = process.env.G06A_MONGO_DATABASE_PREFIX ?? `passhub_g06a_jest_${process.pid}`;
...
try {
  ...
  await run('docker', [
    ...
    '-e', `G06A_MONGO_URI=${uri}`, '-e', `G06A_MONGO_DATABASE_PREFIX=${databasePrefix}`,
    nodeImage, 'sh', '-lc',
    'node --version && npm --version && npm run build && node --experimental-vm-modules node_modules/jest/bin/jest.js --config jest.g06a.integration.config.cjs',
  ]);
} catch (error) {
  failure = error;
} finally {
  try {
    ...
    else process.stderr.write(`G06a cleanup also failed: ${String(cleanupError)}\n`);
  }
}
